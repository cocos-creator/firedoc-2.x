    ...
    DIGESTERS = {
      ...
      'param': function (tagname, value, target, block) {
        ...
        if (type) {
          ...
          var item = this.classitems.filter(function (i) {
            return i.is_type && i.name === type;
          })[0];
          // finded the type
          if (item && item.params) {
            result.props = item.params;
          }
          result.type = type;
        }
        ...
        result.description = Y.localize(result.description);
      ...
      'class': function (tagname, value, target, block) {
        ...
        // localize
        host.description = Y.localize(host.description);
        return host;
      },
      ...
      'property': function (tagname, value, target, block) {
        ...
        // localize the description
        target.description = Y.localize(target.description);
      },
      'method': 'property',
      'attribute': 'property',
      'config': 'property',
      ...
      'example': function (tagname, value, target, block) {
        ...
        if (value) {
          ...
          if (linkMatch && linkMatch.length === 2) {
            var examplePath = process.cwd() + '/' + linkMatch[1];
    ...
    _resolveFor: function (value) {
      if (value.indexOf('.') === -1) {
        Y.each(this.classes, function (i) {
          if (i.shortname === value && i.namespace) {
    ...
    **/
    ...
    processblock: function (block) {
      ...
      if (host) {
        ...
        target._global = this.get(CURRENT_CLASS) === '';
    ...
    transform: function (commentmap) {
      var self = this,
        ...
        classinherits = self.classinherits = [];
      ...
      Y.each(classes, function (clazz, name) {
        ...
        if (clazz.file) {
          ...
          if (modules[clazz.module] && modules[clazz.module].itemtype !== 'main') {
