...
var Locals = {
        ...
        _.each(member.params, function (param) {
          ...
          if (param.props && param.props.length > 0) {
            _.each(param.props, function (prop) {
              prop.description = self.markdown(prop.description);
            });
          }
        });
      }
  ...
  getExpandIterator: function (parent) {
    ...
    return function (item) {
      ...
      // get the description first sentence
      if (item.description) {
        var result, match, summary = '';
        if (item.i18n['LANG'] === 'zh') {           
          match = item.description.match(/.+?\。/);
        }
        else if (item.i18n['LANG'] === 'en') {
          match = item.description.match(/.+?\.[\s\n]/);
        }
        if (!match) {
          match = item.description.match(/(.+\w)\s*\n/);
        ...
        summary = utils.safetrim(result);
        item.summary = summary.replace(/<br\/>/g, "");
      }
