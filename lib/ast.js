...
var AST = {
  ...
  onblock: function (block) {
    ...
    // run digiesters
    _.each(this.context.block.digesters, function (ctx) {
      var ret = ctx.fn.call(this, 
        ctx.name, 
        ctx.value, 
        this.context.block.target, 
        this.context.block.self);
    ...
    if (this.context.block.host) {
      ...
      if (ns) {
        ...
        target.namespace = ns;
        target.parent = this.classes[target.clazz] || this.modules[target.module];
      }
  ...
  'class': function (tagname, value, target, block) {
    ...
    block.forEach(function (def) {
      if (def.tag === 'namespace') {
        ...
        if (value.indexOf(utils.safetrim(def.value) + '.') === -1) {
          value = name;
          namespace = utils.safetrim(def.value);
        }
      }
      if (def.tag === 'extends') {
