...
YUI.add('doc-builder', function (Y) {
  ...
  Y.DocBuilder.prototype = {
    ...
    buildMember: function (member, forceBeMethod ,parent) {
      member.description = this._parseCode(member.description);
      ...
      if (member.example) {
        ...
        member.example = member.example.map(function (v) {
          return this._parseCode(this.markdown(v))
        }).join('');
      }
      if (parent) {
      ...
      if (member.itemtype === 'method' || forceBeMethod) {
        member.methodDisplay = this.getMethodName(member.name, member.params);
      ...
      if (member.itemtype === 'attribute') {
        member.emit = this.options.attributesEmit;
      }
      return member;
    },
    ...
    get classes () {
      ...
      return Object.keys(self.data.classes).map(
        function (name) {
          ...
          clazz.description = self._parseCode(
            self.markdown(clazz.description || ''));
    ...
    renderModule: function (cb, data, layout) {
      ...
      Y.prepare([DEFAULT_THEME, themeDir], self.getProjectMeta(), function (err, opts) {
        ...
        opts.meta.projectAssets = '../assets';
        opts.meta.projectLogo = self._resolveUrl(self.data.project.logo, opts);
        opts.meta._moduleItems = self.data.classitems.filter(function (i) {
          return i.module === opts.meta.moduleName;
        }).filter(function (i) {
          return !i['class'];
        });
        opts.meta.moduleItems = {
          'methods': [],
          'properties': [],
          'attrs': [],
          'events': []
        };
        opts.meta._moduleItems.forEach(function (i) {
        ...
        if (data.classes && Object.keys(data.classes).length) {
          opts.meta.moduleEnums = [];
          opts.meta.moduleClasses = [];
          Y.each(Object.keys(data.classes), function (name) {
            var i = self.data.classes[name];
            if (i) {
              var item = {name: i.name, displayName: i.name};
              if (i.is_enum) {
                opts.meta.moduleEnums.push(item);
              } else {
                opts.meta.moduleClasses.push(item);
              }
            }
          });
          opts.meta.moduleEnums.sort(self.nameSort);
    ...
    writeModules: function (cb, layout) {
      layout = layout || 'main';
      var self = this, stack = new Y.Parallel();
      stack.html = [];
      stack.view = [];
      ...
      Y.each(self.data.modules, function (v) {
        ...
        self.renderModule(function (html, view) {
          ...
          var outPath = path.join(self.options.outdir, 'modules', v.name);
          if (Y.options.useMarkdown) {
            outPath += '.md';
          } else {
            outPath += '.html';
    ...
    renderClass: function (cb, data, layout) {
      ...
      Y.prepare([DEFAULT_THEME, themeDir], self.getProjectMeta(), function (err, opts) {
        ...
        var mainLayout = opts.layouts[layout];
        var tmpl = '{{>classes}}';
        // when it's enum class, use enums.handlerbars template
        if (data.is_enum) {
          tmpl = '{{>enums}}';
        }
    ...
    writeClasses: function (cb, layout) {
      ...
      Y.each(self.data.classes, function (v) {
        ...
        self.renderClass(stack.add(function (html, view) {
          var dir = v.is_enum ? 'enums' : 'classes';
          stack.html.push(html);
          stack.view.push(view);
          if (self.options.dumpview) {
            Y.Files.writeFile(
              path.join(self.options.outdir, 'json', dir + '_' + v.name + '.json'),
              JSON.stringify(view),
              stack.add(noop)
            );
          }
          var outPath = path.join(self.options.outdir, dir, v.name);
          if (Y.options.useMarkdown) {
            outPath += '.md';
          } else {
            outPath += '.html';
