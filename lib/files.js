...
function copyDir (src, dest, overwrite, callback) {
 ...
 * @param {Boolean} [overwrite=false] Whether or not to overwrite destination files if they already exist.
 * @param {Callback} callback The callback to be executed when complete.
 * @param {Error} callback.err The Error returned from Node
 **/
function copyFile (src, dest, overwrite, callback) {
  // Allow callback as third arg.
  if (typeof overwrite === 'function') {
    callback = overwrite;
    overwrite = null;
  ...
  **/
function copyPath (src, dest, overwrite, callback) {
  ...
  **/
  ...
  function readFileSync(file, enc) {
    var data = fs.readFileSync(file, enc);
    return data;
  }

  Y.Files.readFileSync = readFileSync;
});
