...
const DIGESTERS = {
  'param': function (tagname, value, target, block) {
    ...
    if (!value) {
      ...
      console.warn('param name/type/descript missing: ' + block);
      return;
    }
    ...
    if (!name) {
      if (value && value.match(/callback/i)) {
        ...
        console.warn('Fixing missing name for callback:' + stringlog(block));
        name = 'callback';
        type = 'Callback';
      } else {
        this.warnings.push({
        ...
        console.warn('param name missing: ' + value + ':' + stringlog(block));
        name = 'UNKNOWN';
      }
    }
