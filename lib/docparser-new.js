...
var ParserContext = {
  ...
  set mainModule (val) {
    ...
    if (this.module === name) {
      if (_.has(AST.modules, name)) {
        if (AST.modules[name].tag === 'main') {
          write = false;
        }
        if (write) {
          AST.modules[name] = _.extend(AST.modules[name], val);
        }
      } else {
        if (val._main) {
          AST.modules[name] = val;
        }
      }
    }
  },
  ...
  set module (val) {
    ...
    var clazz = AST.classes[this.clazz];
    if (clazz) {
      if (clazz.module !== val) {
        if (AST.modules[this.module]) {
          delete AST.modules[clazz.module].submodules[clazz.submodules];
          delete AST.modules[clazz.module].classes[clazz.name];
        }
        if (clazz.submodule && AST.modules[clazz.submodule]) {
          delete AST.modules[clazz.submodule].submodules[clazz.submodule];
          delete AST.modules[clazz.submodule].classes[clazz.name];
        }
        clazz.module = val;
        if (AST.modules[val]) {
          AST.modules[val].submodules[clazz.submodule] = 1;
          AST.modules[val].classes[clazz.name] = 1;
        }
        if (clazz.submodule && AST.modules[clazz.submodule]) {
          AST.modules[clazz.submodule].module = val;
        }
      }
    }
    if (!_.has(AST.modules, val)) {
      AST.modules[val] = {
        'name': val,
        'classes': {},
        'submodules': {},
  ...
  set clazz (val) {
    ...
    var ns, clazz;
    var name = val;
    if (!_.has(this.classes, val)) {
      ns = ParserContext.namespace;
...
const DIGESTERS = {
  ...
  'module': function (tagname, value, target, block) {
    ParserContext.module = value;
    var go = true;
    _.some(block, function (o) {
      if (utils.safetrim(o.tag) === 'submodule') {
        go = false;
        return true;
      }
    });
    if (go) {
      if (!ParserContext.mainModule) {
        ParserContext.mainModule = {
          tag: tagname,
          name: value,
          file: target.file,
          line: target.line,
  ...
  'submodule': function (tagname, value, target, block) {
    ParserContext.submodule = value;
    var host = this.modules[value],
      clazz = ParserContext.clazz,
  ...
  'class': function (tagname, value, target, block) {
    ...
    block.forEach(function (def) {
      ...
      if (def.tag === 'extends') {
        var extended = tils.safetrim(def.value);
    ...
    parent = ParserContext.submodule;
    if (parent) {
