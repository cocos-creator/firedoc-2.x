  ...
  **/
  ...
  function getPages(dir) {
    ...
    fs.readdirSync(dir).forEach(function (filename) {
      ...
      var extname = Y.options.useMarkdown ? '.mdt' : '.handlebars';
      if (path.extname(filename) === extname && Y.Files.isFile(filePath)) {
        pages[path.basename(filename, extname)] = fs.readFileSync(filePath, Y.charset);
      }
    });
    cache[dir] = pages;
  ...
  var getProjectData = function (dir) {
    ...
    while (dirs.length && !projectData) {
      ...
      if ((packageData && projectData) || (packageData && packageData.yuidoc)) {
        console.log(packageData, projectData);
